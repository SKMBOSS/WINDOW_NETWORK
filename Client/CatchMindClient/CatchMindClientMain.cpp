...
HINSTANCE g_hInst;
HWND hWnd;
LPCTSTR lpszClass = TEXT("CatchMindClient");
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdParam, int nCmdShow)
{
	...
	CatchMindClient::GetInstance()->Init(hWnd, g_hInst, g_sock);
	...
	while (true)
	{
		...
		else
		{
			...
			if (hForeWindow == hWnd)
			{
				CatchMindClient::GetInstance()->Update();
			}
			CatchMindClient::GetInstance()->Render();
		}
	}
	CatchMindClient::GetInstance()->Release();
	closesocket(g_sock);
	WSACleanup();
...
void ProcessSocketMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case FD_READ:
	{
		...
		CatchMindClient::GetInstance()->ProcessPacket(szBuf, retval);
	}
	break;
	case FD_CLOSE:
		closesocket(wParam);
